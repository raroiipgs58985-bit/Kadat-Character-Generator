...
global.window = {};
require("../src/domain/regiment-origin.js");
const api = window.KADAT_REGIMENT_CHARACTER_LINK_INTERNALS;
  ...
  regimentTypes: [
    {
      ...
      equipmentText:
        "лазкарабин (главное оружие) и четыре батареи и магнокль на игрового персонажа; «Василиск» на отделение или миномёт на каждых двух игровых персонажей; вокс-станция на отделение.",
    },
  ],
  trainingDoctrines: [],
  equipmentDoctrines: [],
...
const aliasedCatalog = catalog;
assert.equal(
  aliasedCatalog.homeworlds[0].characteristicsText,
  catalog.homeworlds[0].characteristicText,
);
assert.equal(
  aliasedCatalog.origins[0].skillsText,
  catalog.origins[0].skillText,
);
assert.equal(
  aliasedCatalog.commanders[0].talentsText,
  catalog.commanders[0].talentText,
);
...
assert.equal(
  mechanics.choices.filter((choice) => choice.kind === "stat").length,
  1,
);
assert.equal(
  mechanics.choices.filter((choice) => choice.kind === "stat-option").length,
  1,
);
assert.equal(
  mechanics.choices.filter((choice) => choice.kind === "skill").length,
  1,
);
assert.equal(
  mechanics.choices.filter((choice) => choice.kind === "equipment").length,
  1,
);
assert.equal(
  mechanics.skills.filter((skill) => skill === "Язык (Низкий готик)").length,
  3,
);
assert.equal(
  mechanics.skills.filter((skill) => skill === "Язык (Высокий готик)").length,
  3,
);
...
assert.deepEqual(
  mechanics.selected.drawbacks.map((entry) => entry.id),
  ["drawback-a", "drawback-b"],
);

const legacyMechanics = api.buildMechanics(aliasedCatalog, {
  ...snapshot,
  drawbackIds: undefined,
  drawbackId: "drawback-a",
});
assert.deepEqual(
  legacyMechanics.selected.drawbacks.map((entry) => entry.id),
  ["drawback-a"],
);
...
for (const choice of mechanics.choices) {
  ...
  if (choice.kind === "equipment")
    selections[choice.id] = "Василиск на отделение";
}
