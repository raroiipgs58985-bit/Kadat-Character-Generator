(() => {
  ...
  const currentArchetype = () => DATA.findByRoll(state.type === "race" ? DATA.raceArchetypes : DATA.beastArchetypes, state.archetypeRoll);
  const currentFeature = () => DATA.findByRoll(DATA.features, state.featureRoll);

  function injectModeButton() {
    const switcher = document.querySelector("#registry-mode-switch");
    if (!switcher || switcher.querySelector('[data-registry-mode="xeno"]')) return;
    const button = document.createElement("button");
    button.type = "button";
    button.dataset.registryMode = "xeno";
    button.innerHTML = `<span>XENOS</span><strong>КСЕНО</strong>`;
    switcher.appendChild(button);

    switcher.addEventListener("click", event => {
      const mode = event.target.closest("[data-registry-mode]")?.dataset.registryMode;
      if (!mode) return;
      if (mode === "xeno") setXenoMode(true);
      else setXenoMode(false);
    });
  }
  ...
  function buildShell() {
    ...
    refs.progress.forEach(button => button.addEventListener("click", () => goToStep(Number(button.dataset.xenoGo))));
    refs.prev.addEventListener("click", () => goToStep(state.step - 1));
    refs.next.addEventListener("click", () => { if (validateStep(state.step)) goToStep(state.step + 1); });
  ...
  function setXenoMode(active) {
    ...
    if (active) {
      document.querySelectorAll("[data-registry-mode]").forEach(button => button.classList.toggle("is-active", button.dataset.registryMode === "xeno"));
      characterBuilder?.classList.add("hidden");
      characterResult?.classList.add("hidden");
      regiment?.classList.add("hidden");
      refs.section?.classList.remove("hidden");
      document.body.dataset.registryMode = "xeno";
      if (mastheadTitle) mastheadTitle.textContent = "REGISTRUM XENOS K-100";
      if (mastheadSubtitle) mastheadSubtitle.textContent = "Когитатор классификации ксено-форм";
      if (protocol) protocol.textContent = "ПРОТОКОЛ: КСЕНО-ФОРМУЛЯР";
      renderStep();
  ...
  function hideValidation() { refs.validation.textContent = ""; refs.validation.className = "message hidden"; }

  function resetState() {
    state.step = 0; state.type = "race"; state.name = ""; state.baseStats = null; state.baseWounds = null;
    state.formRoll = null; state.size = 0; state.archetypeRoll = null; state.featureRoll = null;
    state.seed = Math.floor(Math.random() * 0xFFFFFFFF); state.profile = null;
    state.choices = { exoticStats: [], warlikeReward: "combat-talents", warlikeTalents: ["", ""], machineTrait: false, apexTalents: ["", ""], keenSense: "Зрение", extraLimbs: "legs", breathWeapon: "spike" };
    refs.resultView.classList.add("hidden"); refs.form.classList.remove("hidden"); refs.progress[0].parentElement.classList.remove("hidden");
    renderStep();
  }
  ...
  function renderType() {
    ...
    refs.stage.querySelector("#xeno-name").addEventListener("input", event => { state.name = event.target.value; });
    refs.stage.querySelector("#xeno-type").addEventListener("change", event => {
      state.type = event.target.value; state.baseStats = null; state.baseWounds = null; state.archetypeRoll = null; state.profile = null; renderStep();
    });
  }
  ...
  function renderStats() {
    ...
    refs.stage.querySelector("#xeno-roll-stats").addEventListener("click", () => {
      const base = ENGINE.baseProfile(state.type, Math.random); state.baseStats = base.stats; state.baseWounds = base.wounds; state.seed = Math.floor(Math.random() * 0xFFFFFFFF); renderStep();
    });
  }
  ...
  function renderBody() {
    ...
    refs.stage.querySelector("#xeno-roll-form").addEventListener("click", () => { state.formRoll = roll100(); state.seed = Math.floor(Math.random() * 0xFFFFFFFF); renderStep(); });
    refs.stage.querySelector("#xeno-size").addEventListener("change", event => { state.size = Number(event.target.value); state.seed = Math.floor(Math.random() * 0xFFFFFFFF); renderStep(); });
  }

  function archetypeChoices(archetype) {
    if (!archetype) return "";
    if (archetype.id === "exotic") return `<div class="choice-grid"><label>Первая характеристика<select data-xeno-choice="exotic-0">${statOptions(state.choices.exoticStats[0])}</select></label><label>Вторая характеристика<select data-xeno-choice="exotic-1">${statOptions(state.choices.exoticStats[1])}</select></label></div>`;
    if (archetype.id === "warlike") return `<div class="choice-grid"><label>Воинственное преимущество<select data-xeno-choice="warlike-reward"><option value="combat-talents"${state.choices.warlikeReward === "combat-talents" ? " selected" : ""}>Два боевых таланта</option><option value="natural-weapon"${state.choices.warlikeReward === "natural-weapon" ? " selected" : ""}>Природное оружие</option></select></label>${state.choices.warlikeReward === "combat-talents" ? `<label>Боевой талант 1<input data-xeno-choice="warlike-talent-0" value="${escapeHtml(state.choices.warlikeTalents[0])}"></label><label>Боевой талант 2<input data-xeno-choice="warlike-talent-1" value="${escapeHtml(state.choices.warlikeTalents[1])}"></label>` : ""}</div>`;
    if (archetype.id === "mechanical") return `<label class="xeno-check"><input type="checkbox" data-xeno-choice="machine"${state.choices.machineTrait ? " checked" : ""}> Получить свойство Машина (1/2 бВН)</label>`;
    if (archetype.id === "apex") return `<div class="choice-grid"><label>Любой талант 1<input data-xeno-choice="apex-talent-0" value="${escapeHtml(state.choices.apexTalents[0])}"></label><label>Любой талант 2<input data-xeno-choice="apex-talent-1" value="${escapeHtml(state.choices.apexTalents[1])}"></label></div>`;
    return "";
  }
  ...
  function bindArchetypeChoices() {
    refs.stage.querySelectorAll("[data-xeno-choice]").forEach(control => {
      const save = () => {
        ...
        if (id === "exotic-0" || id === "exotic-1") state.choices.exoticStats[Number(id.at(-1))] = control.value;
        if (id === "warlike-reward") state.choices.warlikeReward = control.value;
        if (id.startsWith("warlike-talent-")) state.choices.warlikeTalents[Number(id.at(-1))] = control.value;
        if (id === "machine") state.choices.machineTrait = control.checked;
        if (id.startsWith("apex-talent-")) state.choices.apexTalents[Number(id.at(-1))] = control.value;
  ...
  function renderArchetype() {
    ...
    refs.stage.querySelector("#xeno-roll-archetype").addEventListener("click", () => { state.archetypeRoll = roll100(); state.seed = Math.floor(Math.random() * 0xFFFFFFFF); renderStep(); });
    bindArchetypeChoices();
  }

  function featureChoices(feature) {
    if (!feature) return "";
    if (feature.id === "keen-sense") return `<label class="primary-field">Обострённое чувство<select data-feature-choice="keen"><option>Зрение</option><option>Слух</option><option>Обоняние</option><option>Вкус</option><option>Осязание</option></select></label>`;
    if (feature.id === "extra-limbs") return `<label class="primary-field">Тип конечностей<select data-feature-choice="limbs"><option value="legs"${state.choices.extraLimbs === "legs" ? " selected" : ""}>Многоногое</option><option value="arms"${state.choices.extraLimbs === "arms" ? " selected" : ""}>Многорукое</option></select></label>`;
    if (feature.id === "breath-weapon") return `<label class="primary-field">Дыхательное оружие<select data-feature-choice="breath"><option value="spike">Выстрел шипом</option><option value="acid">Кислотное дыхание</option><option value="toxic">Токсичное дыхание</option><option value="fire">Огненное дыхание</option></select></label>`;
    return "";
  }

  function buildProfile() {
    return ENGINE.buildProfile({
      type: state.type, name: state.name.trim() || (state.type === "race" ? "Безымянная ксено-раса" : "Безымянный ксено-зверь"),
      baseStats: state.baseStats, baseWounds: state.baseWounds, formRoll: state.formRoll, size: state.size,
      archetypeRoll: state.archetypeRoll, featureRoll: state.featureRoll, seed: state.seed, choices: state.choices
    });
  }

  function renderFeature() {
    const feature = currentFeature();
    const canPreview = state.baseStats && state.formRoll && state.archetypeRoll && state.featureRoll;
    ...
    refs.stage.querySelector("#xeno-roll-feature").addEventListener("click", () => { state.featureRoll = roll100(); state.seed = Math.floor(Math.random() * 0xFFFFFFFF); renderStep(); });
    const keen = refs.stage.querySelector('[data-feature-choice="keen"]'); if (keen) { keen.value = state.choices.keenSense; keen.addEventListener("change", () => { state.choices.keenSense = keen.value; renderStep(); }); }
    const limbs = refs.stage.querySelector('[data-feature-choice="limbs"]'); if (limbs) limbs.addEventListener("change", () => { state.choices.extraLimbs = limbs.value; renderStep(); });
    const breath = refs.stage.querySelector('[data-feature-choice="breath"]'); if (breath) { breath.value = state.choices.breathWeapon; breath.addEventListener("change", () => { state.choices.breathWeapon = breath.value; renderStep(); }); }
  }
  ...
  function validateStep(step) {
    hideValidation();
    if (step === 0 && !state.name.trim()) return showValidation("Укажите название ксеноса или расы.");
    if (step === 1 && !state.baseStats) return showValidation("Сначала бросьте характеристики.");
    if (step === 2 && !state.formRoll) return showValidation("Сначала бросьте к100 на физическую форму.");
    if (step === 3 && !state.archetypeRoll) return showValidation("Сначала бросьте к100 на архетип.");
    const archetype = currentArchetype();
    if (step >= 3 && archetype?.id === "exotic" && new Set(state.choices.exoticStats.filter(Boolean)).size !== 2) return showValidation("Экзотическая раса должна выбрать две разные характеристики.");
    if (step >= 3 && archetype?.id === "warlike" && state.choices.warlikeReward === "combat-talents" && state.choices.warlikeTalents.filter(value => value.trim()).length !== 2) return showValidation("Укажите два боевых таланта или выберите природное оружие.");
    if (step >= 3 && archetype?.id === "apex" && state.choices.apexTalents.filter(value => value.trim()).length !== 2) return showValidation("Для верховного зверя укажите два любых таланта.");
    if (step === 4 && !state.featureRoll) return showValidation("Сначала бросьте к100 на особенность.");
    return true;
  }

  function validateAll() { for (let step = 0; step < 5; step += 1) if (!validateStep(step)) { state.step = step; renderStep(); return false; } return true; }

  function goToStep(step) {
    const target = Math.max(0, Math.min(4, step));
    if (target > state.step && !validateStep(state.step)) return;
    state.step = target; renderStep(); refs.section.scrollIntoView({ behavior: "smooth", block: "start" });
  }
  ...
  function renderStep() {
    ...
    refs.progress.forEach((button, index) => { button.classList.toggle("is-active", index === state.step); button.classList.toggle("is-complete", index < state.step); });
    refs.prev.disabled = state.step === 0; refs.next.hidden = state.step === 4; refs.submit.hidden = state.step !== 4;
    refs.status.textContent = `Этап ${state.step + 1} из 5`;
    refs.rollStatus.textContent = state.step === 2 ? `к100: ${state.formRoll ?? "—"}` : state.step === 3 ? `к100: ${state.archetypeRoll ?? "—"}` : state.step === 4 ? `к100: ${state.featureRoll ?? "—"}` : "к100: —";
  }

  function renderResult() {
    const profile = state.profile;
    refs.form.classList.add("hidden"); refs.progress[0].parentElement.classList.add("hidden"); refs.resultView.classList.remove("hidden");
    refs.resultView.querySelector("#xeno-code").textContent = `XEN-${String(Math.floor(Math.random() * 1000000)).padStart(6, "0")}`;
    refs.result.innerHTML = `<div class="character-card"><div><h3>${escapeHtml(profile.name)}</h3><p class="muted">${profile.type === "race" ? "Ксено-раса" : "Ксено-зверь"} • ${escapeHtml(profile.form?.name ?? "—")} • ${escapeHtml(profile.archetype?.name ?? "—")}</p></div>
      ...
      <div><h3>Характеристики</h3><table class="result-table"><thead><tr><th>Хар.</th><th>Значение</th><th>Бонус</th></tr></thead><tbody>${DATA.stats.map(stat => `<tr><th>${stat}</th><td>${profile.stats[stat]}</td><td>${Math.floor(profile.stats[stat] / 10)}</td></tr>`).join("")}</tbody></table></div>
    ...
    if (profile.type === "race") {
      const save = () => {
        const race = ENGINE.toCharacterRace(profile); DATA.saveRace(race); window.dispatchEvent(new CustomEvent("kadat:xeno-race-saved", { detail: race })); return race;
      };
      refs.result.querySelector("#save-xeno-race").addEventListener("click", event => { save(); event.currentTarget.textContent = "Раса сохранена"; });
      refs.result.querySelector("#create-xeno-character").addEventListener("click", () => {
        const race = save();
        document.querySelector('[data-registry-mode="character"]')?.click();
        window.KADAT_SELECT_XENO_RACE?.(race);
      });
    }
    refs.resultView.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  function init() { injectModeButton(); buildShell(); renderStep(); }
